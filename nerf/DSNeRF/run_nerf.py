...
#         raw = run_network(pts, fn=run_fn)
    ...
    # NOTE modified from 11 to 10
    # if sigma_loss is not None and ray_batch.shape[-1] > 11:
    if sigma_loss is not None and ray_batch.shape[-1] > 10:
        depths = ray_batch[:,8]
    ...
    for i in trange(start, N_iters):
        ...
        if args.depth_loss:
            ...
            if args.weighted_loss:
                ...
                # NOTE: I modified to change decrease the keypoints
                depth_loss = img2mse(depth_col, target_depth)
                # depth_loss = img2mse(depth_col[:10], target_depth[:10])
        sigma_loss = 0
        if args.sigma_loss:
