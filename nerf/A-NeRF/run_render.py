...
# ZJU n_bullet=10
# Redenrpeople n_bullet=18
def init_catalog(args, n_bullet=18):
    ...
    # NeuralBody
    nb_subjects = ['315', '377', '386', '387', '390', '392', '393', '394', '996', '55']
    # TODO: hard-coded: 6 views
    nb_idxs = np.arange(len(np.concatenate([np.arange(1, 31), np.arange(400, 601)])) * 6)
    # bullet_idxs = [i for i in range(1, 90, 5)]
    # 10 views training for synthesia
    # bullet_idxs = [3, 7, 22, 24, 54, 78, 80, 109, 112, 56]
    bullet_idxs = [i for i in range(1, 10, 1)]
    nb_dict = lambda subject: {'data_h5': f'data/zju_mocap/{subject}_test_h5py.h5',
                               'val': set_dict(nb_idxs, length=1, skip=1),
                               'mesh': set_dict([i for i in range(1, 10, 1)], length=1, skip=1),
                               'bullet': set_dict(bullet_idxs, 
                                   n_bullet=n_bullet, center_kps=True, center_cam=False),
                               }
