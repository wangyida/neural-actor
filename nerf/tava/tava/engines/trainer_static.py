...
class Trainer(AbstractEngine):
    ...
    def run(self) -> float:  # noqa
        ...
        for step in range(self.init_step, self.max_steps + 1):
            ...
            if is_main_thread and step % self.cfg.print_every == 0:
                ...
                dep_loss = sum([s["loss_dep"] for s in stats_trace]) / len(
                    stats_trace
                )
                ...
                self.tb_writer.add_scalar("train_dep_loss", dep_loss, step)
    ...
    def train_step(self, data, lr):
        ...
        rgb, depth, _, _, delta, t_vals, weights = ret[-1]
        loss = F.mse_loss(rgb, pixels)
        psnr = compute_psnr_from_mse(loss)
        idx_body = (depths < 4.0).nonzero()
        idx_evn = (depths >= 4.0).nonzero()
        if self.cfg.depth=='l2':
            # Norm-2 distance
            # loss_dep = torch.mean((depth[idx_body] - depths[idx_body]) ** 2)
            loss_dep = F.mse_loss(depth[idx_body], depths[idx_body])
            loss_dep += F.mse_loss(depth[idx_evn], depths[idx_evn])
        elif self.cfg.depth=='l2_body':
            # Norm-2 distance
            # loss_dep = torch.mean((depth[idx_body] - depths[idx_body]) ** 2)
            loss_dep = F.mse_loss(depth[idx_body], depths[idx_body])
            loss_dep += 0.1 * F.mse_loss(depth[idx_evn], depths[idx_evn])
        elif self.cfg.depth=='kl':
            # KL divergence
            err = 1.0
            loss_dep = -torch.log(weights) * torch.exp(-(depth[idx_body] - depths[idx_body]) ** 2 / (2 * err)) * delta
            loss_dep = weights * torch.exp(-(t_vals - depths[:,None]) ** 2 / (2 * err)) * delta
            loss_dep = torch.mean(torch.sum(loss_dep, dim=1))
        elif self.cfg.depth=='ds':
            err = 1.0
            # loss_dep = -torch.log(weights) * torch.exp(-(t_vals - depths[:,None]) ** 2 / (2 * err)) * delta
            loss_dep = -torch.log(weights[idx_body[0]]) * torch.exp(-(t_vals[idx_body[0]] - depths[idx_body[0]][:,None]) ** 2 / (2 * err)) * delta[idx_body[0]]
            print(loss_dep)
            # removing weights
            loss_dep = torch.mean(torch.sum(loss_dep, dim=1))
        else:
        ...
        if len(ret) > 1:
            ...
            rgb_c, depth_c, _, _, delta_c, t_vals_c, weights_c = ret[0]
            loss_c = F.mse_loss(rgb_c, pixels)
            psnr_c = compute_psnr_from_mse(loss_c)
            """
            loss_dep += torch.mean((depth_c[idx_body] - depths[idx_body]) ** 2)
            """
        else:
            loss_c = 0.0
            psnr_c = 0.0
