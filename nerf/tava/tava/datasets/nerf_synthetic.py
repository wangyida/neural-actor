...
def _load_renderings(root_fp: str, subject_id: str, split: str):
    ...
    with open(
        os.path.join(data_dir, "transforms_{}_2v.json".format(split)), "r"
    ) as fp:
        meta = json.load(fp)
    images = []
    depths = []
    ...
    for i in range(len(meta["frames"])):
        ...
        fname_d = os.path.join(data_dir, frame["file_path"] + ".exr")
        depth = imageio.imread(fname_d)[:,:,0]
...
class SubjectLoader(CachedIterDataset):
    ...
    SUBJECT_IDS = ["chair", "drums", "ficus", "hotdog", "lego", "materials", "mic", "renderpeople"]
    ...
    def preprocess(self, data):
        ...
        if self.num_rays is not None:  # usually this is in the training phase
            ...
            depths = depths * alpha.reshape(resolution)[ray_indices]
            rays = namedtuple_map(
