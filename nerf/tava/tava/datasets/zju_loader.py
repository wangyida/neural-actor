...
def _dataset_view_split(parser, split):
    # _train_camera_ids = [0, 6, 12, 18, 24, 30, 36, 42, 48, 54, 60, 66, 72, 78, 84]
    # _train_camera_ids = [0, 6, 12, 18]
    # _train_camera_ids = [6, 24, 54, 78]
    if parser.training_view == 10:
        _train_camera_ids = list(np.array([3, 7, 22, 24, 54, 56, 78, 80, 109, 112]) - 1)
    elif parser.training_view == 20:
        _train_camera_ids = list(np.array([3, 7, 22, 24, 54, 56, 78, 80, 109, 112, 67, 5, 131, 132, 139, 140, 147, 148, 155, 156]) - 1)
    elif parser.training_view == 40:
        _train_camera_ids = list(np.array([3, 7, 22, 24, 54, 56, 78, 80, 109, 112, 67, 5, 131, 132, 139, 140, 147, 148, 155, 156, 153, 145, 17, 130, 30, 120, 133, 154, 19, 128, 16, 13, 77, 110, 86, 66, 69, 41, 61, 27]) - 1)
    elif parser.training_view == 80:
        _train_camera_ids = list(np.array([3, 7, 22, 24, 54, 56, 78, 80, 109, 112, 67, 5, 131, 132, 139, 140, 147, 148, 155, 156, 153, 145, 17, 130, 30, 120, 133, 154, 19, 128, 16, 13, 77, 110, 86, 66, 69, 41, 61, 27, 29, 101, 119, 138, 49, 76, 8, 89, 158, 149, 12, 123, 100, 152, 21, 65, 91, 36, 85, 52, 103, 90, 141, 134, 68, 150, 23, 39, 33, 159, 70, 127, 18, 73, 114, 59, 87, 2, 142, 71]) - 1)
    elif parser.training_view == 130:
        _train_camera_ids = list(np.array([3, 7, 22, 24, 54, 56, 78, 80, 109, 112, 67, 5, 131, 132, 139, 140, 147, 148, 155, 156, 153, 145, 17, 130, 30, 120, 133, 154, 19, 128, 16, 13, 77, 110, 86, 66, 69, 41, 61, 27, 29, 101, 119, 138, 49, 76, 8, 89, 158, 149, 12, 123, 100, 152, 21, 65, 91, 36, 85, 52, 103, 90, 141, 134, 68, 150, 23, 39, 33, 159, 70, 127, 18, 73, 114, 59, 87, 2, 142, 71, 1, 129, 4, 6, 9, 10, 11, 137, 146, 20, 151, 25, 157, 31, 32, 160, 34, 40, 43, 44, 47, 48, 50, 51, 53, 55, 62, 64, 72, 74, 75, 79, 81, 82, 84, 88, 99, 104, 105, 106, 107, 108, 111, 113, 115, 116, 118, 124, 125, 126]) - 1)
    if split == "all":
        ...
        # 30 test cameras
        camera_ids = list(np.array([14, 15, 26, 28, 35, 37, 38, 42, 45, 46, 57, 58, 60, 63, 83, 92, 93, 94, 95, 96, 97, 98, 102, 117, 121, 122, 143, 144, 135, 136]) - 1)
        # camera_ids = list(np.array([15]) - 1)
    elif split == "test":
        # camera_ids = [0]
        # NOTE renderpeople
        # camera_ids = [8]
...
class SubjectLoader(CachedIterDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.parser = SubjectParser(subject_id=subject_id, training_view=training_view, root_fp=root_fp)
    ...
    def preprocess(self, data):
        ...
        # rgba, depth, rays = data["rgba"], data["depth"], data["rays"]
        ...
        if self.num_rays is not None:  # usually this is in the training phase
            ...
            # depths = depth.reshape(resolution)[ray_indices]
            rays = namedtuple_map(
    ...
    def fetch_data(self, index):
        ...
        rgba = np.concatenate(
            [
                ...
                self.parser.load_mask(frame_id, camera_id, trimap=False)[
                    ..., None
                ],
            ],
            axis=-1,
        ...
        self.parser.WIDTH = rgba.shape[1]
        self.parser.HEIGHT = rgba.shape[0]
