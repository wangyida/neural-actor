...
def extract_video(videoname, path, start, end, step):
	...
	# resize image
        ...
        if int(videoname.split(os.sep)[-1][:-4]) == 3:
            frame = cv2.rotate(frame, cv2.ROTATE_90_CLOCKWISE)
        cv2.imwrite(join(outpath, '{:04d}.jpg'.format(cnt-start)), frame, [int(cv2.IMWRITE_JPEG_QUALITY), 98])
        # cv2.imwrite(join(outpath, '{:04d}.jpg'.format(cnt)), frame)
        # cv2.imwrite(join(outpath, '{:04d}.jpg'.format(cnt-start)), frame[:,:,0], [int(cv2.IMWRITE_JPEG_QUALITY), 98])
    video.release()
    return base
...
def convert_from_openpose(path_orig, src, dst, annotdir, ext):
    ...
    for inp in tqdm(inputlist, desc='{:10s}'.format(os.path.basename(dst))):
        ...
        imgname = annotname.replace(annotdir, 'images').replace('.json', '.' + ext)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--with_img', action='store_true')
