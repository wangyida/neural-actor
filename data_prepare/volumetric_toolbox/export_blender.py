...
# Unauthorized copying and distribution is strictly prohibited.
try:
    import bpy
except ModuleNotFoundError:
    print(
        "This program needs to be executed through blender like this:\n"
        "```\nblender --background --python export_cameras_blender.py -- --output_blend /tmp/cameras.blend --help\n```"
    )
    exit()
...
sys.path.append(os.fspath(Path(__file__).resolve().parent))
...
def main():
    ...
    parser.add_argument("--image_name", type=str, default="{camera_name}_00000.jpg", help="File name for frames.")
    parser.add_argument(
        "--no_root",
        action="store_true",
    ...
    for camera in cameras:
        ...
        assert np.isclose(camera.fx_pixel, camera.fy_pixel)  # Square pixels are assumed
        ...
        if args.images:
            ...
            image = load_image(camera_image_name, args.images, recursive=False)
            background_image = blender_camera.data.background_images.new()
            background_image.image = image
            image.filepath_raw = f"//{os.path.relpath(args.images / camera_image_name, Path(bpy.data.filepath).parent)}"
    ...
    if args.abc:
        ...
        )  # NOTE(mr-synth): It would be nicer to infer this automatically
